...
describe('ContextEngine', () => {
  it('supports the interface with a deterministic Friday Merge mock', () => {
    const engine: ContextEngine = new MockContextEngine();
    const result = engine.analyze('Nora: just merged the auth rewrite into main\nKai: on a friday??\nLeo: fearless behavior 💀\nKai: enjoy your weekend');
    expect(result?.tone).toEqual(['playful', 'sarcastic']);
    expect(result?.register).toBe('technical');
  });

  it('does not infer context for unsupported input', () => {
    expect(new MockContextEngine().analyze('unrelated fixture')).toBeUndefined();
  });
});
