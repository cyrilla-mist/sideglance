...
Production Worker is `sideglance-worker-production`. The previously deployed version had health pass but three unclassified HTTP 502 responses on AI smoke; this transport migration is implemented locally and requires redeployment. Configuration is `CONTEXT_ENGINE_MODE=ai`, `MODEL_TRANSPORT=cloudflare_google_openai_passthrough`, the existing `MODEL_NAME`, and the account ID plus two credential secrets. The request is sent to `/v1/{account_id}/default/google-ai-studio/v1beta/openai/chat/completions` with the direct `{MODEL_NAME}` value. `PRODUCTION_MODEL_FINALIZATION_PENDING_EVALUATION` remains true.
