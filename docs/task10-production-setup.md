...
Production Worker is `sideglance-worker-production`. The previously deployed versions had health pass but AI smoke 502 responses on unified compat and the superseded OpenAI passthrough. This native transport migration is implemented locally and requires redeployment. Configuration is `CONTEXT_ENGINE_MODE=ai`, `MODEL_TRANSPORT=cloudflare_google_native`, the existing `MODEL_NAME`, and the account ID plus two credential secrets. The request is sent to `/v1/{account_id}/default/google-ai-studio/v1/models/{MODEL_NAME}:generateContent` with the existing model value. `PRODUCTION_MODEL_FINALIZATION_PENDING_EVALUATION` remains true.
