...
function Invoke-SideglanceHttpCurl {
  ...
  try {
    ...
    if ($Method -eq 'POST') { $curlArgs = @('--header', 'Content-Type: application/json', '--data-binary', '@-') + $curlArgs }
    ...
    $psi.Arguments = ($curlArgs | ForEach-Object { '"' + ([string]$_).Replace('"', '\"') + '"' }) -join ' '
    $process = New-Object System.Diagnostics.Process
    $process.StartInfo = $psi
    if (-not $process.Start()) { throw 'curl process could not be started.' }
    if ($Method -eq 'POST') { $utf8Body = [System.Text.Encoding]::UTF8.GetBytes($BodyJson); $process.StandardInput.BaseStream.Write($utf8Body, 0, $utf8Body.Length); $process.StandardInput.BaseStream.Flush() }
    ...
    $process.Dispose()
    $started.Stop(); $status = $null; $statusMatch = [regex]::Match(($statusText -join '').Trim(), '[0-9]{3}$'); if ($statusMatch.Success) { $status = [int]$statusMatch.Value }
