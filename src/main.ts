...
function bind() {
  ...
  document.querySelector('#decode-again')?.addEventListener('click', () => { void submit(inputText, additionalContext); });
}

async function submit(text: string, context?: string) {
  if (!text.trim()) { response = { type: 'failed', errorCode: 'invalid_request', message: 'Paste a phrase or conversation first.' }; state = 'failed'; render(); return; }
  ...
  catch (error) { response = { type: 'failed', errorCode: 'invalid_response', message: error instanceof DecodeClientError ? error.message : 'Something interrupted the reading. Please try again.' }; state = 'failed'; }
  render();
}
