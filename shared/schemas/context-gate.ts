...
export type ContextGateIssue = { path: string; reason: 'missing_field' | 'wrong_type' | 'invalid_enum' | 'invalid_shape'; expected: string; receivedType?: string; receivedValue?: string };
...
export function diagnoseContextGateResult(value: unknown): { valid: boolean; issues: ContextGateIssue[] } {
  ...
  if (!sufficiency || typeof sufficiency !== 'object' || Array.isArray(sufficiency)) issues.push({ path: 'sufficiency', reason: 'invalid_shape', expected: 'object', receivedType: describeType(sufficiency) });
  ...
  if (result.status === 'needs_context') {
    ...
    else if ((result.question.match(/\?/g) ?? []).length > 1) issues.push({ path: 'question', reason: 'invalid_shape', expected: 'one specific question', receivedType: 'multiple_questions' });
  }
